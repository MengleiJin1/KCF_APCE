...
%   Joao F. Henriques, 2014
	...
	%detail
	resize_image = (sqrt(prod(target_sz)) >= 100);  %diagonal size >= threshold。。。prod用来求向量或矩阵的元素乘积
	if resize_image,
		pos = floor(pos / 2);
...
% 	window_sz = 2 .^ nextpow2(window_sz);
	...
	positions = zeros(numel(img_files), 2);  %to calculate precision。。。numel求出图像中元素的个数（不知道2是什么意思），zeros生成numel * numel的全0矩阵
	...
	for frame = 1:numel(img_files),
		...
		if frame > 1,
			...
			switch kernel.type
			case 'gaussian',
				kzf = gaussian_correlation(zf, model_xf, kernel.sigma);%此处为互相关，用来得到公式22的Kxz
			case 'polynomial',
		...
		patch = get_subwindow(im, pos, window_sz);%在一帧画面中获得padding大小的图像块patch，位置为上一帧padding的位置
		xf = fft2(get_features(patch, features, cell_size, cos_window));

		%Kernel Ridge Regression, calculate alphas (in Fourier domain)
		switch kernel.type
		case 'gaussian',
			kf = gaussian_correlation(xf, xf, kernel.sigma);%此处为自相关，用来得到公式17分母的Kxx
		case 'polynomial',
			kf = polynomial_correlation(xf, xf, kernel.poly_a, kernel.poly_b);
		case 'linear',
			kf = linear_correlation(xf, xf);
		end
		alphaf = yf ./ (kf + lambda);   %equation for fast training。。。公式17
		...
		if frame == 1,  %first frame, train with a single image
			...
			%subsequent frames, interpolate model。。。给跟踪器提供一些记忆，不至于更新过于剧烈
         ...
         global counts ;
            ...
            beta1 = 0.5;
            beta2 = 0.9;
            sprintf('current_Fmax = ',curr_Fmax);
